          ...
          {!state.loading && !state.error && state.articles.length > 0 && (
            <>
                    ...
                    <article key={article.id} className="article-card">
                      <div className="article-card__image">
                        <img 
                          src={thumbnailUrl} 
                          alt={title} 
                          onError={(e) => { e.target.src = '/assets/images/placeholder-article.jpg'; }}
                        />
                      </div>
              ...
              <nav className="articles-pagination" aria-label="Articles pagination">
                <button 
                  className={`articles-pagination__btn articles-pagination__btn--prev ${!state.pagination.hasPrevPage ? 'articles-pagination__btn--disabled' : ''}`}
                ...
                <div className="articles-pagination__info">
                  Page <span>{state.pagination.page}</span> of <span>{state.pagination.totalPages}</span>
                </div>
                <button 
                  className={`articles-pagination__btn articles-pagination__btn--next ${!state.pagination.hasNextPage ? 'articles-pagination__btn--disabled' : ''}`}
